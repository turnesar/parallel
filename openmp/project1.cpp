	...
	float * rs = new float [NUMTRIALS];
        ...
        for( int t = 0; t < NUMTRIES; t++ )
        {
				...
				{	
					...
					//if it is less than we missed it darn....
					if(d>=0){ 
						// hits the circle:
						...
						//okay so we hit the circle, but if less than 0 we engulfed it
						if(tmin>=0){
							...
							//yes we hit the circle! But if it went up it t < 0
							if(t>=0){
								numHits++;
								//woot case D and we hit the plate! 
							}}}//closing brackets for if statements
